...
async function mainState() {
	const countState = await prisma.state.count();
	if (countState > 0) {
		console.log("Estados ya existentes, saltando seed.");
		return;
	}

	const sample: Prisma.stateCreateInput[] = [
		{
			name: "Activo",
		},
		{
	...
	await prisma.state.createMany({ data: sample });

	console.log("Seed completado: estados insertados.");
}
